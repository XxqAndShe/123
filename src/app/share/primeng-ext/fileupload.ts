...
@Component({
    ...
    template: `
        ...
        <div *ngIf="uploadType==='file'" >
             <div *ngIf="multiple==='single'" class="ui-fileupload-buttonbar ui-widget-header ui-corner-top" style="display: inline-block;border:none;background:none;">
                <button type="button" [label]="chooseLabel" icon="fa-plus" pButton class="ui-fileupload-choose" 
                (click)="onChooseClick($event, fileinput)" [disabled]="disabled || hasFiles()"> 
                    <input #fileinput type="file" (change)="onFileSelect($event)" [accept]="accept" [disabled]="disabled">
                    <input #fileinput type="file" (change)="onFileSelect($event)" [multiple]="multiple" *ngIf="multiple!=='single'" [accept]="accept" [disabled]="disabled">
                </button>
                <button *ngIf="!auto" type="button" [label]="uploadLabel" icon="fa-upload" pButton (click)="upload()" [disabled]="!hasFiles()"></button>
                <button *ngIf="!auto" type="button" [label]="cancelLabel" icon="fa-close" pButton (click)="clear()" [disabled]="!hasFiles()"></button>
                <p-templateLoader [template]="toolbarTemplate"></p-templateLoader>
                <span *ngFor="let f of uploadedFiles">{{f.name.substr(0,18)}}</span>
            </div>
             <div *ngIf="multiple!=='single'" class="ui-fileupload-buttonbar ui-widget-header ui-corner-top" style="display: inline-block;border:none;background:none;">
                <button type="button" [label]="chooseLabel" icon="fa-plus" pButton class="ui-fileupload-choose"
                 (click)="onChooseClick($event, fileinput)" [disabled]="disabled || hasFiles()">
             ...
             <p-progressBar [value]="progress" [showValue]="false" *ngIf="hasFiles()" class="file-progress"></p-progressBar>
         </div>
    `
})
export class FileUpload implements OnInit, AfterContentInit {

    @Input() uploadType: string; //上传类型
